...
qsub -t 1-"$num_common_var_genes" -l mem_free=2G -l h_rt=05:00:00 -sync y -o "$project_root/logs/out/filt_vcfs_acc.out" -e "$project_root/logs/err/filt_vcfs_acc.err" "$excavate_vcf_creation" "$output_dir" "$param_file" "$input_metadata"
...
if [[ "$OFF_TARGETS" == "True" ]]; then
    qsub -t 1-"$num_common_var_genes" -l mem_free=1G -l h_rt=08:00:00 -sync y -o "$project_root/logs/out/excavate_acc.out" -e "$project_root/logs/err/excavate_acc.err" "$run_excavate_script" "$output_dir" "$param_file" "$input_metadata"
else
    qsub -t 1-"$num_common_var_genes" -l mem_free=1G -l h_rt=04:45:00 -sync y -o "$project_root/logs/out/excavate_acc.out" -e "$project_root/logs/err/excavate_acc.err" "$run_excavate_script" "$output_dir" "$param_file" "$input_metadata"
fi
...
qsub -t 1-"$num_genes_w_guides" -l mem_free=2G -l h_rt=04:00:00 -sync y -o "$project_root/logs/out/guide_filtering_acc.out" -e "$project_root/logs/err/guide_filtering_acc.err" "$guide_based_filtering" "$output_dir" "$param_file" "$genes_w_guides"
...
qsub -l mem_free=2G -l h_rt=05:00:00 -o "$project_root/logs/out/hets_acc_prepam_${RUN_NAME}.out" -e "$project_root/logs/err/hets_acc_prepam_${RUN_NAME}.err" "$get_targeted_hets_prePAM" "$output_dir/prePAM_hets" "$param_file" "$gene_info" "$filtered_vcf_dir"
...
qsub -l mem_free=2G -l h_rt=05:00:00 -sync y -o "$project_root/logs/out/hets_acc.out" -e "$project_root/logs/err/hets_acc.err" "$get_targeted_hets" "$output_dir/excavate/het_individuals" "$param_file" "$genes_w_guides" "$excavate_output_dir" "$filtered_vcf_dir"
