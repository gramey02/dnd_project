...
#         qsub -t 1-"$num_batches" -l mem_free=1G -l h_rt=00:45:00 -sync y -o "$project_root/logs/out/excavate_excision_batched.out" -e "$project_root/logs/err/excavate_excision_batched.err" "$run_excavate_script" "$output_dir" "$param_file" "$batch_metadata"
...
# qsub -t 1-"$num_excavate_genes" -l mem_free=1G -l h_rt=00:45:00 -sync y -o "$project_root/logs/out/excavate_excision.out" -e "$project_root/logs/err/excavate_excision.err" "$run_excavate_script" "$output_dir" "$param_file" "$excavate_gene_info"
