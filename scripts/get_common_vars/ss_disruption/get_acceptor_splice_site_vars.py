...
def main():
    ...
    for gene, transcript_dict in acceptorSNP_ranges_byGene.items():
        ...
        if len(transcript_list)>0:
            ...
            t1_df['start_end'] = t1_df[['acceptor_range_start','acceptor_range_end']].apply(tuple,axis=1)
            t1_acc_loc_list = list(t1_df['start_end'])
            ...
            for transcript2, t2_df in transcript_dict.items():
                if len(t1_acc_loc_list)!=0:
                    if transcript1!=transcript2:
                        ...
                        t2_copy['start_end'] = t2_copy[['acceptor_range_start','acceptor_range_end']].apply(tuple,axis=1)
                        t2_acc_loc_list = list(t2_copy['start_end'])
                        
                        # create a product to get every combination of exon ranges that can be compared
                        range_combos = list(product(t2_acc_loc_list, t1_acc_loc_list))
                        ...
                        t1_acc_loc_list = list(o_regions['overlapping_regions'])
                else:
                    break
        else:
            t1_acc_loc_list=[]
