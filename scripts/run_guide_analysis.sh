...
qsub -cwd -l mem_free=5G -l h_rt=03:00:00 -o "$project_root/logs/out/non_excision_guides.out" -e "$project_root/logs/err/non_excision_guides.err" "$non_excision_guides_script" "$project_root/$output_dir" "$param_file"
...
qsub -cwd -l mem_free=5G -l h_rt=03:00:00  -o "$project_root/logs/out/excision_guides.out" -e "$project_root/logs/err/excision_guides.err" "$excision_guides_script" "$project_root/$output_dir" "$param_file"
